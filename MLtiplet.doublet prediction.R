...
a = 1
if(a==1){
	file="/users/immune-rep/mfj169/SINGLE_CELL_RNA_SEQ/10X_PIPELINE/Samples_10X_v3_pre_CITEseq_cellcell1.txt"
	p <- as.matrix(read.csv(file, head=T, sep="\t"))
	p=p[-1,]
	sample_id = as.character(p[,2])
	sample_run_id = as.character(p[,"Sample.Name"])
	directory = as.character(p[,"Location.of.GEX"])
	BCR.location = as.character(p[,"Location.of.BCR"])
	TCR.location = as.character(p[,"Location.of.TCR"])
	Overall_sample_group = as.character(p[,"Patient"])
	Site = as.character(p[,"Sample.type"])
	batch = "CITE_CC1"
	out_dir = "/well/immune-rep/users/mfj169/10X_GENOMICS/INFILTRIMMUNE/CITE_CC/"
	out_dir_raw = "/well/immune-rep/users/mfj169/10X_GENOMICS/INFILTRIMMUNE/CITE_CC/"
	PLOTS = "PLOTS/"
}
